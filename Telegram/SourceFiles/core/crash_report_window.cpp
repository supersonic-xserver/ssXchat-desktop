...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start ssXchat Desktop!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time ssXchat Desktop was not closed properly."_q);
	} else {
		_label.setText(u"Last time ssXchat Desktop crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST OFFICIAL VERSION OF SSXCHAT DESKTOP"_q);
	connect(&_getApp, &QPushButton::clicked, [=] {
		QDesktopServices::openUrl(u"https://github.com/supersonic-xserver/ssXchat-desktop"_q);
	});
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time ssXchat Desktop was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
